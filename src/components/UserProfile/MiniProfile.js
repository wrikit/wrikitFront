...
const MiniProfile = props => {
  ...
  axios.post(
    "http://localhost:8000/main/get-mini-profile/",
  ...
  .then(res => {
    if (res.data.result) {
      ...
      setImage(`http://localhost:8000/media/user_profile/${res.data.profileImg}`);
  ...
  return {loading} ? (
    <div className="mini-profile" ref={profileRef}>
      {Boolean(message.length) && (<div className="profile-message">{message}</div>)}
      <div className="main">
